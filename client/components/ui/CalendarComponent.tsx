...
const CalendarComponent = () => {
    ...
    const calendarEventss = Object.values(
    ...
    const calendarEvents = calendarEventss.filter(
        event => event.user_id === userId
    )
    const events: any[] = [] // Array to store the events

    // console.log("Current events in store:", events);

    //console.log("Current events store:", calendarEvents);
    const [noEventModalVisible, setNoEventModalVisible] =
    ...
    const handleDayPress = (day: { dateString: string }) => {
        ...
        const event = Array.isArray(calendarEvents)
            ? calendarEvents.find(
                  (calendarEvent: CalendarEvent) =>
                      calendarEvent.date === selectedDay
              )
            : undefined
        ...
        const markedDates = events.reduce((acc: any, event: CalendarEvent) => {
            ...
            const highlightColor =
                activityColors[
                    event.activity.type as keyof typeof activityColors
                ] || 'gray'
            if (!isDuplicate) {
